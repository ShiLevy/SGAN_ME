...
class Sampler:
    ...
    def __init__(self, main_dir=None,CaseStudy=0,seq = 3,ndraw=10000,thin = 1,  nCR = 3, 
                 ...
                 device='cpu',res_calc = 0,percentile=0, tempering = 0, T = 10, burnin = 20000,
                 weighted_err=0,delayed_err=0,stage=5000,err_scale=0,minJRS=0.01):
...
#%% Defining inversion and simulation parameters
        ...
        if self.CaseStudy>=2 and self.CaseStudy<5:   
            ...
            ''' add the model error parameters as well'''
            ...
            if self.CaseStudy==2: #accounting for model error WGAN
                
                DNN.res_calc = res_calc	                       #calculate for reduced mean model =1, regular =0 
                DNN.err_inv = True
                
                if DNN.res_calc == 1:
                    DNN.gpath_error=Extra.main_dir+'/netG_moderr_res.pth'
                else:
...
#                DNN.real_mean = np.load('/home/slevy/Documents/boise_experiment/models_n_moderrs/meanmat_calc_WGAN_moderrs.npy')
#                DNN.real_std = np.load('/home/slevy/Documents/boise_experiment/models_n_moderrs/stdmat_calc_WGAN_moderrs.npy')
                ...
                MCMCPar.n=MCMCPar.ub.shape[1]
                if Extra.err_correction:
                    Extra.correction = np.load('./mean_correctionFWSR.npy')

            #Load measurements
            file_name='./'+str(percentile)+'percentile/True_data_sig0_5.npy'
            ...
            Measurement.True_model = np.load('./'+str(percentile)+'percentile/True_model.npy') #only for the purpose of plotting results, not used in the inversion
            
            # with open(file_name, 'rb') as fin:
            #         tmp=torch.load(fin, map_location=torch.device('cpu'))
            ...
            if self.CaseStudy==3: #accounting for model error covariance matrix (Hansen et al. 2014)
                MCMCPar.lik=5
                MCMCPar.n_err = 800
                folder = './moderrs_FWSR_1E9/'   #model errors folder
        ...
        elif self.CaseStudy==5:     #inverting the model error
            ...
            #DNN.real_mean = np.load('./meanmat_TI.npy')
            #DNN.real_std = np.load('./stdmat_TI.npy')
            ...
            #Load measurements
            file_name='./'+str(percentile)+'percentile/True_error.npy'
            Measurement.MeasData=np.load(file_name)
...
#%% Initialization
    ...
    def _init_sampling(self):
        ...
        if self.MCMCPar.save_tmp_out==True:
            with open('./'+str(Extra.percentile)+'percentile/c'+str(Extra.casestudy)+'/out_tmp.pkl','wb') as f:
...
#            start_time = time.time()
            ...
            for gen_number in range(0,self.MCMCPar.steps):
                ...
                # if tempering is used, scale jr-scale with K
                # if self.MCMCPar.tempering:
                #     self.MCMCPar.jr_scale = self.MCMCPar.jr_scale_init * self.OutDiag.K[self.MCMCVar.iteration-2,1]
            ...
            if MCMCPar.tempering:
                if (self.MCMCVar.Iter/self.MCMCPar.seq) <= self.MCMCPar.burnin and (self.MCMCVar.Iter/self.MCMCPar.seq)%100==0 and self.MCMCPar.jr_scale>MCMCPar.minJRS:               
                    # check if the acceptance rate is lower than 25%, if it is decrease K (incourage high acceptance rate during burn-in period)
                    if self.OutDiag.AR[self.MCMCVar.iteration-1,1] < 20:
                        ...
                        self.OutDiag.jr_scale[self.MCMCVar.iteration-1,:] = np.concatenate((np.array([self.MCMCVar.Iter]).reshape((1,1)), np.array([self.MCMCPar.jr_scale]).reshape((1,1))), axis=1)
                    # Decrease the temperature with iterations during burn-in period
        ...
        if self.MCMCPar.saveout==True:
            ...
            with open('./'+str(Extra.percentile)+'percentile/c'+str(Extra.casestudy)+'/dreamzs_out'+'.pkl','wb') as f:
