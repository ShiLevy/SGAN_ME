...
def RunFoward(X,MCMCPar,Iter,Measurement,ModelName,Extra,DNN=None):
    ...
    if ModelName=='theoretical_case_mvn':
        ...
        for i in range(0,MCMCPar.seq):
            zs[i,:]=X[i,:25].reshape((DNN.nz,DNN.zx,DNN.zy))
        ...
        if MCMCPar.err_scale:
            for i in range(0,MCMCPar.seq):
                m_err[i] = m_err[i] * X[i,DNN.zx*DNN.zy]

        X = m_err.reshape([MCMCPar.seq,-1], order='F')
        ...
        #apply model model error correction
        if Extra.err_correction:
    ...
    elif (ModelName=='linear_gpr_tomo') or (ModelName=='Eikonal'):           
        extra_par=[]
        # Generate realizations
        zs=np.zeros((MCMCPar.seq,DNN.nz,DNN.zy,DNN.zx))
        for i in range(0,MCMCPar.seq):
        ...
        elif ModelName == 'Eikonal':
            extra_par.append(DNN.nx)
            extra_par.append(DNN.ny)
            extra_par.append(Extra.nsource)
            extra_par.append(Extra.nreceiver)
            extra_par.append(Extra.sx)
            extra_par.append(Extra.sz)
            extra_par.append(Extra.rx)
            extra_par.append(Extra.rz)
            extra_par.append(Extra.spacing)
    ...
    if ModelName == 'linear_gpr_tomo':
        ...
        '''why is there model error here?!!! TO CHECK'''
        # if DNN.err_inv == True:           
    ...
    elif (ModelName =='model' or ModelName=='model_error'):
...
def Eikonal(X,n,par):
    ...
    FD_ray, travel_time, err= es.time2d_py(nx, ny, nsource, nreceiver, sx, sz, rz, rx, X, spacing)
...
def mean_n_cov(folder, MCMCPar, Extra):
    '''calculating the bias term and covariance matrix of the model-
       errors according to equations in Hansen et al. (2014)'''
